...
public class FileReportClass {
    ...
    public static void writeServiceReport(Map<Customer, List<ServiceBooking>> bookingMap, String fileName) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            double totalRevenue = bookingMap.values().stream().flatMap(List::stream).mapToDouble(ServiceBooking::getCost).sum();
    ...
    public static void avgCostType(Map<Customer, List<ServiceBooking>> bookingMap) {
        double electricAvg = bookingMap.values().stream().flatMap(List::stream).filter(b -> b.getCustomer().getVehicle() instanceof ElectricVehicle).mapToDouble(ServiceBooking::getCost).average().orElse(0);

        double combustionAvg = bookingMap.values().stream().flatMap(List::stream).filter(b -> b.getCustomer().getVehicle() instanceof CombustionEngineVehicle).mapToDouble(ServiceBooking::getCost).average().orElse(0);
